...
document.addEventListener("DOMContentLoaded", function(event){
		...
		xhr.onreadystatechange = function() {
	    	if (this.readyState == 4 && this.status == 200) {
		      	...
		      	if (resJson.items.length > 0){
		      		...
		      		searchRsElem.innerHTML = '<div style="margin: auto; text-align:center;">Không tìm thấy sản phẩm phù hợp</div>'
		      	}
	...
	searchBtnElem.addEventListener("keypress", function(event){
		activeState(event);
		active = false;
	});
